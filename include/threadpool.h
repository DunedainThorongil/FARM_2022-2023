 ...
 * @file threadpool.h
 *
 * @brief Header file del threadpool
 *
 * @authors Recuperato dalla correzione degli assegnamenti.
 *          Le modifihe sono a cura di Alessandra De Lucrezia 565700
 *
...
typedef struct threadpool_t
{
    ...
    pthread_cond_t cond_connessi;    // usata per notificare che tutti i thread si sono connessi al collector
    pthread_cond_t cond;             // usata per per verdere se la coda è vuota
    pthread_cond_t condFull;         // usata per notificare che la coda è piena
    pthread_cond_t cond_coda;        // usata per notificare quando c'è almeno un posto libero nella coda
    ...
    int connected;    // numero di thread connessi alla socket
 ...
 *
 * @brief aggiunge un task al pool, se ci sono thread liberi il task viene assegnato ad uno di questi, se non ci sono thread liberi e pending_size > 0 allora si cerca di inserire il task come task pendente.
 ...
 * @param arg  argomento della funzione
 *
 * @return (0) se successo, (-1) in caso di fallimento, errno viene settato opportunamente, (1) se coda in fase di uscita.
 */
